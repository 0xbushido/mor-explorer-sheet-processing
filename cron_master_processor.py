...
from sheet_config.google_utils import slack_notification
...
async def run_update_process():
    ...
    slack_notification(f"Starting update process at {start_time}")
    ...
    try:
        ...
        slack_notification(success_message)

    except Exception as e:
        error_message = f"Error in update process: {str(e)}\n{traceback.format_exc()}"
        logger.error(error_message)
        slack_notification(f"Error in update process: {str(e)}")
        raise
